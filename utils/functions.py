...
class Functions():
  ...
  async def get_profile(self, keyboard, user_id, bot):
    ...
    if save:
      await mongo_conn.db.users.update_one({'user_id': user_id}, {'$set': {'attempts_free': user['attempts_free'], 'attempts_pay': user['attempts_pay']}})
