  ...
  async def send_msg(self, id):
    try:
      try:
        ...
        self.die += 1
        self.delete_user += 1
        logging.info('deactivatedUserId: ' + str(id))
      except exceptions.ChatNotFound:
        self.die += 1
        self.acc_not_active += 1
      except exceptions.CantInitiateConversation:
        self.die += 1
        logging.info('cantWriteToUser: ' + str(id))
      except exceptions.CantTalkWithBots:
        self.die += 1
        logging.info('cantWriteToBot: ' + str(id))
      except Exception as e:
        self.die += 1
        self.error += 1
...
async def editor_mailing(chat, user_data, state=None, message_id='', preview=False, mailing=False, select_button='',
  ...
  # TODO Добавить суюда логику отсроченной отправки + доп аргумент в функцию + доп кнопка + придумать логику ввода
